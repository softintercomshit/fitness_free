...
-(void)showNextMonth {
    ...
    [imageCurrentMonth release];
    [imageNextMonth release];
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSMutableArray *weekdays = [[[NSMutableArray alloc] initWithArray:[dateFormatter shortWeekdaySymbols]] autorelease];
...
//    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xf3f3f3"].CGColor);
    ...
    //Draw days
    CGContextSetFillColorWithColor(context, 
                                   [UIColor /*colorWithHexString:@"0x383838"*/lightTextColor].CGColor);
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            CGContextAddRect(context, rectangleGrid);
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0x7283a6"].CGColor);
            CGContextFillPath(context);
            ...
            CGRect rectangleGrid = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth+2,kVRGCalendarViewDayHeight+2);
            CGContextAddRect(context, rectangleGrid);
            CGContextSetFillColorWithColor(context, [UIColor /*colorWithHexString:@"0x383838"*/ lightGrayColor].CGColor);
            CGContextFillPath(context);
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        if (selectedDate && selectedDateBlock==targetBlock) {
            ...
            color  = (UIColor *)[markedColors objectAtIndex:0 ];
        }
...
-(id)init {
    ...
    if (self) {
        ...
        labelCurrentMonth.textColor = [UIColor colorWithHexString:@"0x566990"];
        ...
        //        [self reset];
        [self.labelCurrentMonth release];
    }
    return self;
}
